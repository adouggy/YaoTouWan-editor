...
public class DragSortListView extends ListView {
     ...
     */
    public FloatViewManager mFloatViewManager = null;
    ...
    private void continueDrag(int x, int y) {
        ...
        if (pauseSort) {
            mDragScroller.stopScrolling(true);
        } else if (minY > mLastY && minY > mDownScrollStartY && currentScrollDir != DragScroller.DOWN) {
    ...
    public boolean startDrag(int position, int dragFlags, int deltaX, int deltaY) {
        ...
        if (position == editingPosition) {
            if (floatViewForEditRow != null) {
                v = floatViewForEditRow;
            }
        }
        if (v == null)
...
//        }
        ...
        mDragListener.startDrag(position);
    ...
    private View locateTargetView(int position, int x, int y) {
        final ViewGroup srcItem = (ViewGroup) getChildAt(position + getHeaderViewsCount() - getFirstVisiblePosition());
        if (srcItem != null) {
            for (int i=0; i<srcItem.getChildCount(); i++) {
                View subView = srcItem.getChildAt(i);
                if (subView instanceof SubViewLocater) {
                    SubViewLocater locater = (SubViewLocater) subView;
                    int[] loc = new int[2];
                    getLocationOnScreen(loc);
                    return locater.locateViewAtPosition(x + loc[0], y + loc[1]);
                }
            }
        }
        return null;
    ...
    public void clickItem(final int position, final float x, final float y) {
        clickTime ++;
        if (clickTime == 1) {
    ...
    private View floatViewForEditRow;
    public void makeFloatViewForEditRow(int position) {
        floatViewForEditRow = mFloatViewManager.onCreateFloatView(position);
    }

    public interface SubViewLocater {
    ...
    private void updateFloatView() {
        ...
        if (floatY < topLimit) {
            ...
            // commented by Jason Hsu
            // 这行代码是为了防止越过ListView的下边沿
//            mFloatLoc.y = bottomLimit - mFloatViewHeight;
    ...
    public void setClickListener(ClickListener l) { mClickListener = l; }
    ...
    public interface ClickListener {
        public void click(int position, View targetView);
        public void doubleClick(int position, View targetView);
    }
    ...
    public interface DragSortListener extends DropListener, DragListener, RemoveListener, ClickListener {
    }
